...
Feel free to play the game at: [african-countries](https://africa-game-cbd56.firebaseapp.com/)
...
* [`src/african_countries/`](src/african_countries/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/african_countries/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `african-countries` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/african_countries/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns african-countries.example
  (:require [african-countries.config :as config])
